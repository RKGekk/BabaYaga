...
void GeometryGenerator::CreateSphere(float radius, unsigned int sliceCount, unsigned int stackCount, MeshData& meshData) {
	...
	for (unsigned int i = 1; i <= stackCount - 1; ++i) {
		...
		for (unsigned int j = 0; j <= sliceCount; ++j) {
			...
			v.Position.x = radius * DirectX::XMScalarSin(phi) * DirectX::XMScalarCos(theta);
			v.Position.y = radius * DirectX::XMScalarCos(phi);
			v.Position.z = radius * DirectX::XMScalarSin(phi) * DirectX::XMScalarSin(theta);

			// Partial derivative of P with respect to theta
			v.TangentU.x = -radius * DirectX::XMScalarSin(phi) * DirectX::XMScalarSin(theta);
			v.TangentU.y = 0.0f;
			v.TangentU.z = +radius * DirectX::XMScalarSin(phi) * DirectX::XMScalarCos(theta);
...
void GeometryGenerator::CreateGeosphere(float radius, unsigned int numSubdivisions, MeshData& meshData) {
	...
	for (unsigned int i = 0; i < meshData.Vertices.size(); ++i) {
		...
		float y = meshData.Vertices[i].Position.z;
		...
		// Quadrant I or IV
		if (x >= 0.0f) { if (theta < 0.0f) theta += 2.0f * DirectX::XM_PI; } // in [0, 2*pi); If x = 0, then atanf(y/x) = +pi/2 if y > 0; atanf(y/x) = -pi/2 if y < 0 
		// Quadrant II or III
		else theta += DirectX::XM_PI; // in [0, 2*pi).

		float phi = DirectX::XMScalarACos(meshData.Vertices[i].Position.y / radius);

		meshData.Vertices[i].TexC.x = theta / DirectX::XM_2PI;
		meshData.Vertices[i].TexC.y = phi / DirectX::XM_PI;

		// Partial derivative of P with respect to theta
		meshData.Vertices[i].TangentU.x = -radius * DirectX::XMScalarSin(phi) * DirectX::XMScalarSin(theta);
		meshData.Vertices[i].TangentU.y = 0.0f;
		meshData.Vertices[i].TangentU.z = +radius * DirectX::XMScalarSin(phi) * DirectX::XMScalarCos(theta);
...
void GeometryGenerator::BuildCylinderTopCap(float bottomRadius, float topRadius, float height, unsigned int sliceCount, unsigned int stackCount, MeshData& meshData) {
...
void GeometryGenerator::BuildCylinderBottomCap(float bottomRadius, float topRadius, float height, unsigned int sliceCount, unsigned int stackCount, MeshData& meshData) {
	// Build bottom cap.
	unsigned int baseIndex = (unsigned int)meshData.Vertices.size();
	float y = -0.5f * height;
